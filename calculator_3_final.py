...
class UserData(object):
    ...
    def calculator(self,stuff1): #此时么就传入进来了
        ...
        factor = (shebao['YangLao'] + shebao['YiLiao'] + shebao['GongShang'] + shebao['ShengYu'] + shebao['GongJiJin']+shebao['ShiYe'])
...
def main():
    ...
    try:
        if len(args) != 6:
            ...
            userdata = UserData(userdatafilename)
            userdata.get_userdata()

            index = args.index('-o')
            outputfilename = args[index + 1]
            userdata.dumptofile(outputfilename)
